...
> AES（Advanced Encryption Standard，高级加密标准）是一种**对称加密算法**。属于分组密码，每次处理固定长度的数据块。
>
> - 分组长度：固定为 128 位（16 Byte）
> - 密钥长度：支持 **128、192、256 位**三种，分别称为 AES-128、AES-192、AES-256
> - **轮数**：根据密钥长度不同，进行 10、12 或 14 轮加密。
>
> 
>
> 分组密码的工作模式
>
> - ECB：直接对每个块加密的模式
> - CBC：**让每个明文块在加密前，先与前一个密文块进行异或，再用 AES 加密，第一个明文块则与 IV 异或。**

##### CBC模式

```markdown
- 加密过程：
	假设明文被分成 P1,P2,P3,...,Pn，每块16字节。
	- 需要一个初始向量 IV，长度也是 16 B，通常随机生成，与密文一起传输
	- 加密公式：
		C1 = Ek(P1 ⊕ IV)
		C2 = Ek(P2 ⊕ C1)
		C3 = Ek(P3 ⊕ C2)
		...
		Ci = Ek(Pi ⊕ Ci-1)
	补充：Ek 表示用密钥 k 进行 AES 加密
	
	

- 解密过程：
	先用 AES 解密当前密文块 Ci，得到一个中间值 Dk(Ci)，然后再和前一个密文块 Ci-1 异或，才能得到明文 Pi。
	- 解密公式：
		P1 = Dk(C1) ⊕ IV
		P2 = Dk(C2) ⊕ C1
		...
		Pi = Dk(Ci) ⊕ Ci-1
	补充：Dk 表示用密钥 k 进行 AES 解密



- Padding（填充）
	AES 分组长度是 16 字节，但实际数据长度往往不是 16 的整数倍。所以加密前需要填充，让最后一个块也变成 16 字节。
	最常用的填充标准是 PKCS#7：
		如果缺 3 个字节，就填充 03 03 03；
		如果缺 5 个字节，就填充 05 05 05 05 05；
		如果刚好是 16 的整数倍，就额外填充一整块 10 10 ... 10（16 个 0x10）。
